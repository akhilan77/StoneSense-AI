...
export default function HospitalFederatedLearning() {
  ...
  const [currentModel, setCurrentModel] = useState<Awaited<
    ReturnType<typeof fetchHospitalCurrentModel>
  > | null>(null);
  ...
  useEffect(() => {
    if (
      !live ||
      !activeStatuses.has(live.status) ||
      ['READY', 'COMPLETED', 'FAILED'].includes(live.round_status ?? '')
    )
      return;
  ...
  const isActive = Boolean(
    live && live.round_status && !['READY', 'COMPLETED', 'FAILED'].includes(live.round_status)
  );
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs text-[#101B16]/55">
                <span
                  className={`h-2 w-2 rounded-full ${connected ? 'bg-[#1F6F5C]' : 'bg-[#B3261E]'}`}
                />
              ...
              <p className="mt-1 font-mono text-xs text-[#101B16]/55">
                {hospitalCode ?? 'Hospital identity loading'}
              </p>
            </div>
            <div className="text-left md:text-right">
              ...
              <p
                className={`mt-1 text-sm font-semibold ${live?.status === 'FAILED' ? 'text-[#B3261E]' : 'text-[#1F6F5C]'}`}
              >
                {loading ? 'Loading...' : isActive ? live?.status : 'No active training round'}
              </p>
              {lastSync && (
                <p className="mt-1 text-[11px] text-[#101B16]/45">
                  Last event {new Date(lastSync).toLocaleString()}
                </p>
              )}
            </div>
          </div>
        ...
        <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-[#101B16]/55">
                Stages reflect coordinator events; no training percentage is inferred.
              </p>
            </div>
            {live?.status === 'FAILED' && (
              <span className="rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-[#B3261E]">
                Unable to participate in this round
              </span>
            )}
          </div>
          <div className="mt-5 grid gap-3 md:grid-cols-3 lg:grid-cols-6">
            {lifecycle.map((item, index) => {
              ...
              const current =
                currentIndex === index ||
                (item.status === 'MODEL_RECEIVED' && live?.status === 'TRAINING_COMPLETED');
              return (
                <div
                  key={item.status}
                  className={`rounded-lg border p-3 ${complete ? 'border-[#1F6F5C]/30 bg-[#EBF5F1]' : current ? 'border-[#3B3F8C]/30 bg-[#F0F0FA]' : 'border-[#DDE3DC] bg-[#F7F9F6]'}`}
                >
                  <div
                    className={`mb-2 text-sm font-bold ${complete ? 'text-[#1F6F5C]' : current ? 'text-[#3B3F8C]' : 'text-[#101B16]/35'}`}
                  >
                    {complete ? '✓' : current ? '●' : '○'}
                  </div>
          ...
          {lastEvent && (
            <p className="mt-4 text-xs text-[#101B16]/55">
              Latest event: <span className="font-mono">{lastEvent.event}</span>
            </p>
          )}
        </section>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
            <h3 className="text-sm font-semibold text-[#101B16]">
              Your contribution{' '}
              <span className="ml-1 text-[10px] font-normal uppercase text-[#1F6F5C]">
                Local metrics
              </span>
            </h3>
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <Metric
                ...
                value={local?.duration_sec == null ? '-' : `${local.duration_sec.toFixed(1)}s`}
              />
            </div>
          </section>
          ...
          <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Metric
                label="Version"
                value={currentModel?.current_model_version ?? live?.global_model_version ?? '-'}
                mono
              />
              <Metric
                label="Global round"
                value={String(currentModel?.global_round ?? live?.round ?? '-')}
              />
              ...
              <Metric
                label="Dataset valid"
                value={dataset ? (dataset.is_valid ? 'Validated' : 'Invalid') : '-'}
              />
            </div>
          </section>
        </div>
        ...
        <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
          <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-xs text-[#101B16]/55">
                Only this hospital's persisted local telemetry is shown.
              </p>
            </div>
            <span className="text-xs text-[#101B16]/55">
              {history.length} round{history.length === 1 ? '' : 's'}
            </span>
          </div>
          {history.length === 0 ? (
            <div className="mt-4 rounded-lg border border-dashed border-[#DDE3DC] p-8 text-center text-sm text-[#101B16]/50">
              No completed hospital training rounds yet.
            </div>
          ) : (
            <div className="mt-4 overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="border-b border-[#DDE3DC] text-[10px] uppercase tracking-wider text-[#101B16]/50">
                  <tr>
                    <th className="px-3 py-2">Round</th>
                    <th className="px-3 py-2">Model</th>
                    <th className="px-3 py-2">Samples</th>
                    <th className="px-3 py-2">Local accuracy</th>
                    <th className="px-3 py-2">Local F1</th>
                ...
                <tbody>
                  {history.map((run) => (
                    <tr key={run.id} className="border-b border-[#DDE3DC]/60">
                      <td className="px-3 py-3 font-semibold">#{run.round_number}</td>
                      <td className="px-3 py-3 font-mono text-[10px]">
                        {run.model_version ?? '-'}
                      </td>
                      <td className="px-3 py-3">{run.sample_count.toLocaleString()}</td>
                      <td className="px-3 py-3">{formatMetric(run.train_acc)}</td>
                      <td className="px-3 py-3">{formatMetric(run.train_f1)}</td>
                      <td className="px-3 py-3 text-[#1F6F5C]">Completed</td>
                      <td className="px-3 py-3 text-[#101B16]/55">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
            ...
            <div className="mt-4 flex items-center justify-between rounded-lg bg-[#F7F9F6] p-4">
              <div>
                <p className="text-2xl font-semibold text-[#101B16]">
                  {overview?.active_hospitals_count ?? '-'}
                </p>
                <p className="text-xs text-[#101B16]/55">hospitals participating</p>
              </div>
              <div className="text-right">
                <p className="text-xs font-semibold text-[#1F6F5C]">Your hospital</p>
                <p className="text-xs text-[#101B16]/55">
                  {live?.status === 'COMPLETED' ? '✓ Completed' : (live?.status ?? 'Waiting')}
                </p>
              </div>
            </div>
          </section>
          <section className="rounded-xl border border-[#DDE3DC] bg-white p-6 shadow-xs">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-medium text-[#101B16]">
              ...
              <span>→</span>
              <span className="rounded-md bg-[#EBF5F1] px-3 py-2">Local training</span>
              <span>→</span>
              <span className="rounded-md bg-[#F0F0FA] px-3 py-2">Model update</span>
              <span>→</span>
            ...
            <p className="mt-4 text-xs leading-5 text-[#101B16]/60">
              ...
              are contributed to the coordinator-backed aggregation workflow.
            </p>
          </section>
        </div>
      </div>
